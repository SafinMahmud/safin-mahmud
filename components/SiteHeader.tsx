...
export function SiteHeader() {
  ...
  return (
    <header className="border-b border-rule pb-10 pt-8 md:pb-14 md:pt-12">
      ...
      <h1 className="mt-4 font-serif text-[2.5rem] font-normal leading-[1.05] tracking-[-0.03em] text-fg md:text-[4rem]">
        {site.name}
      </h1>
      <p className="mt-5 max-w-[40rem] text-[1.0625rem] leading-relaxed text-fg md:text-[1.125rem]">
        {site.positioning}
      </p>
      {links.length > 0 ? (
        <ul className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-rule pt-5">
          {links.map((link) => (
            <li key={link.label}>
              <a
                href={link.href}
